...
class TPC(DPC):
    ...
    def calculate_regularization_matrices(self) -> TPCRegularizationMatrices:
        ...
        Qw = pinv(W).T @ self.ddpc_params.Q_horizon @ pinv(W)
    ...
    def calculate_closed_form_solution_matrices(self) -> DPCClosedFormSolutionMatrices:
        ...
        Q_horizon = self.ddpc_params.Q_horizon
        R_horizon = self.ddpc_params.R_horizon
    ...
    def _extract_L22y_matrix(self, L: np.ndarray) -> np.ndarray:
        ...
        indy = (
            np.array(
                [np.arange(1, self.dims.p + 1) + self.dims.mp * i for i in range(self.ddpc_params.tau_f)]
            ).flatten()
            - 1
        )
    ...
    def _calculate_sigma_phi(self, L: np.ndarray) -> np.ndarray:
        ...
        k = self.dims.mp * (self.ddpc_params.tau_p + self.ddpc_params.tau_f)
        P = np.kron(np.eye(self.ddpc_params.tau_f), np.eye(k))
    ...
    def _calculate_sigma_theta(self, L: np.ndarray) -> np.ndarray:
        ...
        """
        tau_f = self.ddpc_params.tau_f
        tau_p = self.ddpc_params.tau_p
