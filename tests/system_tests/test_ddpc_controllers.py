...
def generate_param_comb() -> list:
    ...
    for system in test_maps.SYSTEMS:
        for ctrl in test_maps.CONTROLLERS:
            ...
            for i, ddpc_params in enumerate(test_maps.CTRL_PARAMS[system]):
...
@pytest.mark.parametrize("system, controller_cls,ddpc_params, noise_config, bounds", generate_param_comb())
def test_lti_systems(system, controller_cls, noise_config, ddpc_params, bounds):
    """
    ...
    create_and_test_controller(
        sys, controller_cls, ddpc_params, n_blow_rows, target, **bounds, pid_combo=test_maps.PID_COMBO_CONFIG[system]
    )
